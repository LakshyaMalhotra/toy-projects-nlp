...
def lr_tfidf_model(fold, data_file):
    ...
    valid_df["lr_tfidf_pred"] = preds_proba

    print(f"Accuracy: {accuracy:.4f},  ROC score: {roc:.4f}")
    print("")

    columns = ["id", "kfold", "sentiment", "lr_tfidf_pred"]
...
if __name__ == "__main__":
    ...
    for f_ in range(5):
        ...
        out_df = lr_tfidf_model(f_, file_path)
        fold_outputs.append(out_df)

        print(f"Time elapsed: {(time.time() - start_time):.2f} seconds.")

    final_df = pd.concat(fold_outputs)
    final_df.to_csv("../outputs/lr_tfidf_model.csv", index=False)
    print(final_df.head())
